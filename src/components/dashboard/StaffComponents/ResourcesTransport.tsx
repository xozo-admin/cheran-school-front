...
export const ResourcesTransportPage = () => {
  ...
  const fetchInventory = async () => {
    ...
    try {
      const response = await adminApi.inventory.list(
        selectedStaffType !== 'all' ? selectedStaffType : undefined
      );
  ...
  const deleteInventoryItem = async (itemId: number) => {
    ...
    try {
      const response = await adminApi.inventory.delete(itemId);
      const data = response.data;
  ...
  }, [viewMode, selectedStaffType]);

  // Filter inventory based on search term
  const filteredInventory = inventory.filter(item => 
    item.stock_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.staff_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const inventoryStats = {
    ...
    critical: inventory.filter(item => item.status === 'Critical Low').length,
            ...
            {loading ? (
              <div className="p-8 text-center">
                <div className="inline-flex flex-col items-center">
                  ...
                  <p className={combine("mt-3 text-xs sm:text-sm font-medium", get('text', 'secondary'))}>Loading inventory...</p>
                </div>
              </div>
