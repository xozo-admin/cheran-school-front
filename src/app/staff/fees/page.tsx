...
export default function StaffFeesManagementPage() {
  const reportsRef = useRef<HTMLDivElement>(null);
  const dueReportsRef = useRef<HTMLDivElement>(null);
  const { theme } = useTheme();
  const { get, combine } = useThemeClasses();
  const pathname = usePathname();
  const isFinanceReportsRoute = pathname === '/staff/finance/feereports';
  const showFeesPageTabs = !isFinanceReportsRoute;
  const showReportsPageTabs = isFinanceReportsRoute;
  const canManageFeeStructures = false;

  // ============ STATE MANAGEMENT ============

  // Tab state
  const [activeTab, setActiveTab] = useState<'structure' | 'assign' | 'payments' | 'concessions' | 'auditLogs' | 'reports' | 'dueReports' | 'studentReports' | 'receipts'>(
    isFinanceReportsRoute ? 'reports' : 'structure'
  );
  ...
  const [feeAuditPage, setFeeAuditPage] = useState(1);
  const [feeAuditPageSize, setFeeAuditPageSize] = useState(20);
  const [feeAuditTotal, setFeeAuditTotal] = useState(0);
  const [feeAuditSearch, setFeeAuditSearch] = useState('');
  const [feeAuditAction, setFeeAuditAction] = useState('');
  const [feeAuditSeverity, setFeeAuditSeverity] = useState('');
  const [feeAuditDateFrom, setFeeAuditDateFrom] = useState('');
  const [feeAuditDateTo, setFeeAuditDateTo] = useState('');
  ...
  }, [filterYear, filterClass, filterType]);
  ...
  const assignFeeToClass = async () => {
    if (!canManageFeeStructures) {
      toastError('You do not have permission to assign fees');
      return;
    }
    ...
    try {
      ...
      toastError('Failed to assign fee');
    } finally {
  ...
  const updateFeeStructure = async () => {
    if (!canManageFeeStructures) {
      toastError('You do not have permission to update fee structures');
      return;
    }
    setActionLoading(true);
    try {
  ...
  const deleteFeeStructure = async (feeId: number) => {
    if (!canManageFeeStructures) {
      toastError('You do not have permission to delete fee structures');
      return;
    }
    setActionLoading(true);
    try {
  ...
  const fetchConcessions = useCallback(async (year?: string) => {
    ...
    try {
      const queryParams = new URLSearchParams();
      const academicYear = year || concessionFilters.academic_year || currentAcademicYear;
  ...
  }, [concessionFilters.academic_year, concessionFilters.student_id, currentAcademicYear]);
  ...
  const fetchStudentFeeSummaryReport = async () => {
    if (!studentReportFilters.student_id.trim()) {
    ...
    try {
      const res = await adminApi.fees.feeStudentSummary({
        student_id: studentReportFilters.student_id.trim(),
        academic_year: studentReportFilters.academic_year || undefined
      });
      const data = extractApiPayload(res.data);
  ...
  const isFeePaymentAuditLog = useCallback((log: FeeAuditLog) => {
    const action = String(log.action || '').toLowerCase();
    const path = String(log.request_path || '').toLowerCase();
    ...
    const isFeeScope =
      moduleName === 'fees' ||
      operation.includes('fee') ||
      path.includes('/api/fees/') ||
      path.includes('/fees/');
    ...
    const isPaymentOperation =
      ...
      operation.includes('receipt') ||
      detailsText.includes('payment') ||
      detailsText.includes('transaction_id') ||
      detailsText.includes('paid_amount') ||
      detailsText.includes('razorpay');

    const isNonPaymentFeeOperation =
      action.includes('concession') ||
      action.includes('structure') ||
      action.includes('assign') ||
      operation.includes('concession') ||
      operation.includes('structure') ||
      operation.includes('assign');

    return isFeeScope && isPaymentOperation && !isNonPaymentFeeOperation;
  }, []);

  const fetchFeePaymentAuditLogs = useCallback(async (
    page: number = feeAuditPage,
    overrides?: {
      pageSize?: number;
      days?: number;
      search?: string;
    ...
    try {
      const effectivePageSize = overrides?.pageSize ?? feeAuditPageSize;
      const effectiveDays = overrides?.days ?? auditDays;
      const effectiveSearch = overrides?.search ?? feeAuditSearch;
      const effectiveAction = overrides?.action ?? feeAuditAction;
      const effectiveSeverity = overrides?.severity ?? feeAuditSeverity;
      const effectiveDateFrom = overrides?.dateFrom ?? feeAuditDateFrom;
      const effectiveDateTo = overrides?.dateTo ?? feeAuditDateTo;
      const hasDateRange = !!effectiveDateFrom && !!effectiveDateTo;
      const response = await adminApi.audit.staffPaymentLogs({
        page: 1,
        page_size: 200,
      });

      const payload = response?.data?.data || {};
      const logs: FeeAuditLog[] = payload.logs || [];
      const now = new Date();
      const fromDate = hasDateRange ? new Date(`${effectiveDateFrom}T00:00:00`) : null;
      const toDate = hasDateRange ? new Date(`${effectiveDateTo}T23:59:59`) : null;
      const cutoffDate = new Date(now.getTime() - effectiveDays * 24 * 60 * 60 * 1000);
      const normalizedSearch = effectiveSearch.trim().toLowerCase();
      const filteredLogs = logs.filter((log) => {
        if (!isFeePaymentAuditLog(log)) return false;

        const logTime = new Date(log.timestamp);
        if (!Number.isNaN(logTime.getTime())) {
          if (hasDateRange) {
            if (fromDate && logTime < fromDate) return false;
            if (toDate && logTime > toDate) return false;
          } else if (logTime < cutoffDate) {
            return false;
          }
        }
        ...
        if (normalizedSearch) {
          ...
          const searchHaystack = [
            ...
            String(log.request_path || '').toLowerCase(),
            String(log.user_name || '').toLowerCase(),
            detailsText,
          ].join(' ');
        ...
        if (effectiveAction.trim()) {
          const actionNeedle = effectiveAction.trim().toLowerCase();
          const actionHay = String(log.action || '').toLowerCase();
          const opHay = String((log.details || {}).operation || '').toLowerCase();
          if (!actionHay.includes(actionNeedle) && !opHay.includes(actionNeedle)) return false;
        }

        if (effectiveSeverity.trim()) {
          if (String(log.severity || '').toUpperCase() !== effectiveSeverity.toUpperCase()) return false;
        }

        return true;
      });
      setFeeAuditLogs(filteredLogs);
      setFeeAuditTotal(filteredLogs.length);
      setFeeAuditPageSize(effectivePageSize);
      const totalPages = Math.max(1, Math.ceil(filteredLogs.length / effectivePageSize));
      setFeeAuditPage(Math.min(Math.max(1, page), totalPages));
    } catch (error) {
      console.error('Error fetching fee payment audit logs:', error);
      toastError('Failed to fetch fee payment audit logs');
    } finally {
  ...
  const filteredStructures = sortedStructures.filter(structure => {
    const matchesSearch =
      structure.fee_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
      structure.class_name.includes(searchTerm) ||
  ...
  const uniqueYears = Array.from(new Set(feeStructures.map(f => f.academic_year)));
  const uniqueTypes = Array.from(new Set(feeStructures.map(f => f.fee_type)));
  ...
  useEffect(() => {
    const reportTabs = new Set(['reports', 'dueReports', 'studentReports', 'receipts']);
    const feeTabs = new Set(['structure', 'concessions', 'auditLogs']);

    if (isFinanceReportsRoute) {
      if (!reportTabs.has(activeTab)) {
        setActiveTab('reports');
      }
      return;
    }

    if (!feeTabs.has(activeTab)) {
      setActiveTab('structure');
    }
  }, [isFinanceReportsRoute, activeTab]);

  useEffect(() => {
    if (activeTab === 'concessions' && currentAcademicYear) {
      fetchConcessions();
    } else if (activeTab === 'auditLogs') {
      fetchFeePaymentAuditLogs(1);
    } else if (showReportsPageTabs && activeTab === 'reports' && currentAcademicYear) {
      fetchDailyCollection();
    }
  }, [activeTab, currentAcademicYear, fetchConcessions, fetchDailyCollection, fetchFeePaymentAuditLogs, showReportsPageTabs]);
  ...
  const feeAuditTotalPages = Math.max(1, Math.ceil((feeAuditTotal || 0) / feeAuditPageSize));
  const feeAuditCurrentPageLogs = useMemo(() => {
    const start = (feeAuditPage - 1) * feeAuditPageSize;
    const end = start + feeAuditPageSize;
    return feeAuditLogs.slice(start, end);
  }, [feeAuditLogs, feeAuditPage, feeAuditPageSize]);

  const applyFeeAuditFilters = () => {
    fetchFeePaymentAuditLogs(1);
  };

  const clearFeeAuditFilters = () => {
    setAuditDays(30);
    setFeeAuditSearch('');
    setFeeAuditAction('');
    setFeeAuditSeverity('');
  ...
  <div className="flex items-center gap-2">
    <select
      ...
      className={combine(getInputClass())}
    >
    ...
    {canManageFeeStructures && (
      ...
      >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mt-6">
            <div className={getStatsCardClass('green')}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={combine("text-xs font-medium", get('text', 'secondary'))}>Total Fees (Assigned)</p>
            ...
            <div className={getStatsCardClass('blue')}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={combine("text-xs font-medium", get('text', 'secondary'))}>Total Concession</p>
                  <p className={combine("text-lg md:text-xl font-bold mt-1", get('text', 'primary'))}>
            ...
            <div className={getStatsCardClass('amber')}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={combine("text-xs font-medium", get('text', 'secondary'))}>Total Pending</p>
                  <p className={combine("text-lg md:text-xl font-bold mt-1", get('text', 'primary'))}>
            ...
            <div className={getStatsCardClass('indigo')}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={combine("text-xs font-medium", get('text', 'secondary'))}>Collected (Academic Year)</p>
                  <p className={combine("text-lg md:text-xl font-bold mt-1", get('text', 'primary'))}>
            ...
            <div className={getStatsCardClass('purple')}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={combine("text-xs font-medium", get('text', 'secondary'))}>Today Collection</p>
                  <p className={combine("text-lg md:text-xl font-bold mt-1", get('text', 'primary'))}>
        ...
        <div className="mb-6 overflow-x-auto">
          <div className="border-b border-gray-200 dark:border-gray-700 min-w-max">
            <nav className="-mb-px flex space-x-6">
              ...
              {canManageFeeStructures && showFeesPageTabs && (
                <button
        ...
        {/* FEE STRUCTURES TAB */}
        {showFeesPageTabs && activeTab === 'structure' && (
          <div className="animate-fade-in">
            {/* Search & Filters */}
            <div className={getCardGradientClass('green', true)}>
              <div className="flex items-center md:flex-row md:items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className={combine(getInputClass(), "h-12 w-auto min-w-[120px]")}
                  >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div>
                    ...
                    <p className={combine("text-sm mt-1", get('text', 'secondary'))}>
                      {filteredStructures.length} structures found
                    </p>
                  </div>
                ...
                {loading ? (
                  <div className="p-12 text-center">
                    <div className="inline-flex flex-col items-center">
                      <div className={combine(
                        "animate-spin rounded-full h-10 w-10 border-4",
                        theme === 'dark' ? 'border-green-500 border-t-transparent' : 'border-green-600 border-t-transparent'
                      )}></div>
                      <p className={combine("mt-4 text-sm font-medium", get('text', 'secondary'))}>
                    ...
                    <p className={combine("text-sm mb-6 max-w-md mx-auto", get('text', 'secondary'))}>
                      {searchTerm || filterYear !== 'all' || filterClass !== 'all'
                        ...
                        : canManageFeeStructures
                          ...
                          : 'No fee structures available right now'}
                    </p>
                    {canManageFeeStructures && (
                      <button
                        onClick={() => setShowAssignModal(true)}
                  ...
                  <>
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className={combine(
                        ...
                        <tr>
                          {['class_name', 'fee_type', 'academic_year', 'amount', 'due_date'].map((key) => (
                            <th
                              key={key}
                              className={combine(
                      ...
                      <div className={combine("px-4 py-3 border-t", get('border', 'primary'))}>
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
                          <p className={combine("text-xs", get('text', 'tertiary'))}>
                            Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredStructures.length)} of {filteredStructures.length} entries
                          </p>
        ...
        {/* ASSIGN FEE TAB */}
        {canManageFeeStructures && activeTab === 'assign' && (
        ...
        {/* CONCESSIONS TAB */}
        {showFeesPageTabs && activeTab === 'concessions' && (
          <div className="animate-fade-in">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="mb-4">
                      <div className="flex gap-2">
                        <select
                          value={concessionFilters.academic_year}
                          onChange={(e) => {
                            setConcessionFilters({ ...concessionFilters, academic_year: e.target.value });
                            fetchConcessions(e.target.value);
                          }}
                          className={combine(getInputClass(), "text-sm py-2.5 flex-1")}
                        >
                          <option value="">Select Academic Year</option>
                          {academicYears.map(year => (
                            <option key={year.id} value={year.name}>
                  ...
                  {loading ? (
                    <div className="py-8 text-center">
                      <div className="inline-flex flex-col items-center">
                        ...
                        <p className={combine("mt-3 text-sm", get('text', 'secondary'))}>
                          Loading concessions...
                        </p>
                      </div>
                    </div>
        ...
        {/* FEES AUDIT LOGS TAB */}
        {showFeesPageTabs && activeTab === 'auditLogs' && (
          <div className="animate-fade-in">
            <div className={getCardGradientClass('indigo', true)}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                <div>
                  <h3 className={combine("text-lg font-bold", get('text', 'primary'))}>Fee Payment Audit Logs</h3>
                  <p className={combine("text-sm", get('text', 'secondary'))}>
                    Showing payment-related fee audit events with filters and pagination.
                  </p>
                </div>
              </div>
              ...
              {feeAuditLoading ? (
                <div className="py-8 text-center">
                  <div className="inline-flex flex-col items-center">
                    ...
                    <p className={combine("mt-3 text-sm", get('text', 'secondary'))}>Loading audit logs...</p>
                  </div>
                </div>
              ) : feeAuditLogs.length === 0 ? (
                <div className="py-8 text-center">
                  <FaHistory className={combine("text-3xl mx-auto mb-3", get('icon', 'secondary'))} />
                  <p className={combine("text-sm", get('text', 'secondary'))}>No payment-related fee audit logs found.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className={combine("bg-gray-50 dark:bg-gray-800", get('border', 'primary'))}>
                      <tr>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Time</th>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Action</th>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Severity</th>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Performed By</th>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Student/Txn</th>
                        <th className={combine("px-4 py-3 text-left text-xs font-medium uppercase", get('text', 'tertiary'))}>Details</th>
                      </tr>
                    </thead>
                    <tbody className={combine("divide-y", get('border', 'primary'))}>
                      {feeAuditCurrentPageLogs.map((log) => {
                        const details = log.details || {};
                        const studentLabel = String(details.student_id || details.student_name || details.transaction_id || details.txn_id || '-');
                        const detailMessage = String(details.message || details.operation || details.error || details.reason || '-');
                        return (
                          <tr key={log.id} className="hover:bg-[var(--color-bg-hover)]">
                            <td className={combine("px-4 py-3 text-sm", get('text', 'secondary'))}>
                            ...
                            <td className={combine("px-4 py-3 text-sm", get('text', 'primary'))}>
                              {String(details.performed_by || log.user_name || 'System')}
                            </td>
                            <td className={combine("px-4 py-3 text-sm font-mono", get('text', 'secondary'))}>
                              {studentLabel}
                            </td>
                            <td className={combine("px-4 py-3 text-sm", get('text', 'secondary'))}>
                              {detailMessage}
              ...
              <div className={combine("mt-4 pt-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3", get('border', 'primary'))}>
                <p className={combine("text-xs", get('text', 'secondary'))}>
                  Page {feeAuditPage} of {feeAuditTotalPages} • Total Logs: {feeAuditTotal}
                </p>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => fetchFeePaymentAuditLogs(Math.max(1, feeAuditPage - 1))}
                    disabled={feeAuditLoading || feeAuditPage <= 1}
                    className={combine(getSecondaryButtonClass(), "px-3 py-2 text-sm disabled:opacity-50")}
                  >
                    Previous
                  </button>
                  <button
                    onClick={() => fetchFeePaymentAuditLogs(Math.min(feeAuditTotalPages, feeAuditPage + 1))}
                    disabled={feeAuditLoading || feeAuditPage >= feeAuditTotalPages}
                    className={combine(getSecondaryButtonClass(), "px-3 py-2 text-sm disabled:opacity-50")}
                  >
                    Next
                  </button>
        ...
        {/* REPORTS TAB */}
        {showReportsPageTabs && (activeTab === 'reports' || activeTab === 'dueReports') && (
              ...
              <div className={getCardGradientClass('amber', true)}>
                <div className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className={getInputClass()}
                      />
                      <span className={combine("self-center", get('text', 'secondary'))}>to</span>
                      <input
                        type="date"
                        value={dateRange.to}
                  ...
                  <button
                    ...
                    className={combine(getPrimaryButtonClass('amber'), "flex items-center gap-2 h-[50px]")}
                    disabled={loading}
                  >
  ...
  >
    ...
    >
      {classFeeReport.statistics.overdue}
        ...
        {/* STUDENT REPORT TAB */}
        {showReportsPageTabs && activeTab === 'studentReports' && (
            ...
            <div className={getCardGradientClass('indigo', true)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-end">
                  <button
                    onClick={fetchStudentFeeSummaryReport}
        ...
        {/* RECEIPTS TAB */}
        {showReportsPageTabs && activeTab === 'receipts' && (
      ...
      {/* Assign Fee Modal */}
      {canManageFeeStructures && showAssignModal && (
      ...
      {/* Update Fee Modal */}
      {canManageFeeStructures && showUpdateModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-sm">
          <div className={getModalCardClass() + " max-w-md w-full shadow-2xl"}>
            <div className="flex justify-between items-center mb-4">
              <h2 className={combine("text-lg font-bold", get('text', 'primary'))}>
      ...
      {canManageFeeStructures && showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-sm">
          <div className={getModalCardClass() + " max-w-md w-full shadow-2xl"}>
            <div className="text-center">
              <div className={combine(
