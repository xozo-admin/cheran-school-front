...
export default function StaffFinanceFeeReportsPage() {
  const reportsRef = useRef<HTMLDivElement>(null);
  const dueReportsRef = useRef<HTMLDivElement>(null);
  const { theme } = useTheme();
  const { get, combine } = useThemeClasses();
  const showFeesPageTabs = false;
  const showReportsPageTabs = true;
  const canManageFeeStructures = false;

  // ============ STATE MANAGEMENT ============

  // Tab state
  const [activeTab, setActiveTab] = useState<'structure' | 'assign' | 'payments' | 'concessions' | 'reports' | 'dueReports' | 'studentReports' | 'receipts'>('reports');
  ...
  const applyConcession = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        toastError(data.error || data.message || 'Failed to apply concession');
      }
    } catch (error) {
      console.error('Error applying concession:', error);
      toastError('Failed to apply concession');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  }, [activeTab, currentAcademicYear, fetchConcessions, fetchDailyCollection, showReportsPageTabs]);
