...
export default function AllStaffPage () {
  ...
  const roleOptions = [
    ...
    { value: 'hostel_warden', label: 'Hostel Warden', icon: FaSchool, color: 'red' },
  ...
  const fetchStaffStats = async () => {
    try {
      ...
      roleOptions.forEach((role) => {
        counts[role.value] = allStaff.filter((s) => s.role === role.value).length;
      });
      setRoleStats(counts);
    } catch (error) {
  ...
  const validateStaffForm = (isEdit: boolean) => {
    ...
    if (!isEdit && !formData.staff_id.trim()) nextErrors.staff_id = 'Staff ID is required.';
  ...
  const createStaff = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      extra_details: {
        ...
        ...(formData.salary_grade.trim() && { salary_grade: formData.salary_grade.trim() }),
        ...(formData.emergency_contact.trim() && { emergency_contact: formData.emergency_contact.trim() }),
        ...(formData.qualification.trim() && { qualification: formData.qualification.trim() }),
      }
    };
    ...
    if (formData.account_holder_name.trim()) payload.account_holder_name = formData.account_holder_name.trim();
    ...
    try {
      ...
      const requiredApiErrors = Object.fromEntries(
        Object.entries(apiErrors).filter(([key]) => requiredStaffFields.includes(key))
      );
      if (Object.keys(requiredApiErrors).length > 0) setFormErrors(requiredApiErrors);
      const firstError = Object.values(requiredApiErrors)[0]
  ...
  const indexOfFirstItem = totalStaffCount === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  ...
  const handleBulkImageUpload = async () => {
    ...
    try {
      const response = await adminApi.csv.uploadProfileImagesZip('staff', imagesZipFile);
      ...
      if (failedCount === 0) {
        toastSuccess(`Profile image upload completed. Updated ${successCount} staff members.`);
      } else {
            ...
            <div className={getCardGradientClass('indigo')}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={combine("text-lg sm:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>{totalStaff}</p>
                </div>
                <div className={combine(
              ...
              <div key={role.value} className={getCardGradientClass(role.color as any)}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={combine("text-lg sm:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>
                ...
                <div className={getCardGradientClass('emerald')}>
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className={combine(
                        ...
                        <FaFileAlt className={combine(
                          "text-base sm:text-lg",
                          theme === 'dark' ? 'text-emerald-400' : 'text-emerald-600'
                        )} />
                      </div>
                      <div>
                        <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Bulk Upload Staff</h3>
                        <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
                    ...
                    >
                      <FaTimes className={get('icon', 'secondary') + " text-xs sm:text-sm"} />
                    </button>
                  </div>
                  ...
                  <div className={combine("mb-3 sm:mb-4 pt-3 border-t", get('border', 'primary'))}>
                    ...
                    <p className={combine("mt-1 text-xs", get('text', 'tertiary'))}>
                      ZIP should contain image files named as staff IDs (e.g., STF001.jpg, STF002.png).
                    </p>
                  </div>
...
<div className={getCardGradientClass()}>
  ...
  <div className={combine("p-3 sm:p-4 border-b", get('border', 'primary'))}>
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
      <div>
        <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Staff Records</h3>
        <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
          View and manage staff information
        </p>
      </div>
      <div className={combine("text-xs", get('text', 'tertiary'))}>
        Showing {indexOfFirstItem} to {indexOfLastItem} of {totalStaff}
      </div>
    </div>
  </div>
        ...
        {loading ? (
          <div className="p-6 sm:p-8 min-h-[280px] flex items-center justify-center text-center">
            <div className="text-center">
              ...
              <p className={combine("mt-4 sm:mt-6 text-xs sm:text-sm font-medium", get('text', 'secondary'))}>Loading staff...</p>
          ...
          <div className="p-6 sm:p-8 text-center">
            ...
            <h3 className={combine("text-sm sm:text-base font-medium mb-0.5 sm:mb-1", get('text', 'primary'))}>No staff found</h3>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className={combine("font-semibold text-sm", get('text', 'primary'))}>{member.name}</p>
                      <p className={combine("text-xs mt-0.5", get('text', 'tertiary'))}>ID: {member.staff_id}</p>
                    </div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                    <div className="col-span-2">
                      ...
                      <p className={combine("mt-0.5 truncate", get('text', 'secondary'))}>{member.email || 'N/A'}</p>
                      <p className={combine("mt-0.5", get('text', 'secondary'))}>{member.phone}</p>
                    </div>
                    <div>
                      <p className={combine("font-medium", get('text', 'tertiary'))}>Joined</p>
                      <p className={combine("mt-0.5", get('text', 'secondary'))}>
                        {member.joining_date ? new Date(member.joining_date).toLocaleDateString() : 'N/A'}
                      </p>
                    </div>
                    <div>
                      <p className={combine("font-medium", get('text', 'tertiary'))}>Gender</p>
                      <p className={combine("mt-0.5", get('text', 'secondary'))}>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-1.5">
                    <button
                      onClick={() => router.push(`/admin/staff/overview/${member.staff_id}`)}
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-primary)]", get('icon', 'primary') + " text-xs")}
                      title="View Profile"
                    >
                      <FaEye className="text-xs" />
                    </button>
                    <button
                      onClick={() => router.push(`/admin/staff/attendance?staffId=${member.staff_id}&tab=history&redirectedFrom=staff-directory`)}
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-info)]", get('icon', 'primary') + " text-xs")}
                    ...
                    <button
                      onClick={() => {
                        const staffType = normalizeStaffType(member.role);
                        router.push(`/admin/staff/resources?search=${encodeURIComponent(member.name)}&staffType=${encodeURIComponent(staffType)}&redirectedFrom=staff-directory`);
                      }}
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-warning)]", get('icon', 'primary') + " text-xs")}
                      title="View Inventory Usage"
                    >
                      <FaBox className="text-xs" />
                    </button>
                    <button
                      onClick={() => router.push(`/admin/staff/work?search=${encodeURIComponent(member.name)}&filterStaffType=${member.role}&redirectedFrom=staff-directory`)}
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-warning)]", get('icon', 'primary') + " text-xs")}
                    ...
                    <button
                      onClick={() => {
                        ...
                        params.set('redirectedFrom', 'staff-directory');
                        router.push(`/admin/salary?${params.toString()}`);
                      }}
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-success)]", get('icon', 'primary') + " text-xs")}
                    ...
                    <button
                      ...
                      className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-error)]", get('icon', 'primary') + " text-xs")}
                      title="Delete"
                    >
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 relative text-xs sm:text-sm">
            ...
            <tr>
              <th className={combine(
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs sm:text-sm">Staff ID</span>
                  {sortField === 'staff_id' && (sortDirection === 'asc' ? <FaSortUp className={get('accent', 'primary') + " text-xs"} /> : <FaSortDown className={get('accent', 'primary') + " text-xs"} />)}
                </div>
              </th>
              <th className={combine(
                "px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium uppercase tracking-wider cursor-pointer bg-[var(--color-bg-secondary)]",
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {sortField === 'name' && (sortDirection === 'asc' ? <FaSortUp className={get('accent', 'primary') + " text-xs"} /> : <FaSortDown className={get('accent', 'primary') + " text-xs"} />)}
                </div>
              </th>
              <th className={combine(
                "px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium uppercase tracking-wider cursor-pointer bg-[var(--color-bg-secondary)]",
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {sortField === 'role' && (sortDirection === 'asc' ? <FaSortUp className={get('accent', 'primary') + " text-xs"} /> : <FaSortDown className={get('accent', 'primary') + " text-xs"} />)}
                </div>
              </th>
              <th className={combine(
              ...
              <th className={combine(
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {sortField === 'joining_date' && (sortDirection === 'asc' ? <FaSortUp className={get('accent', 'primary') + " text-xs"} /> : <FaSortDown className={get('accent', 'primary') + " text-xs"} />)}
                </div>
              </th>
              <th className={combine(
                "px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium uppercase tracking-wider bg-[var(--color-bg-secondary)]",
          ...
          <tbody className={combine("divide-y", get('border', 'primary'))}>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-3 sm:px-4 py-8 text-center">
                  <div className={combine("inline-block p-3 rounded-full mb-3", theme === 'dark' ? 'bg-indigo-900/30' : 'bg-indigo-100')}>
                    <FaUserTie className={combine("text-xl", theme === 'dark' ? 'text-indigo-400' : 'text-indigo-500')} />
                  </div>
                  ...
                  <p className={combine("text-xs sm:text-sm mb-4", get('text', 'secondary'))}>
                    {searchTerm || filterRole !== 'all' ? 'Try adjusting your search or filters' : 'Add your first staff member to get started'}
                  </p>
                  {!searchTerm && filterRole === 'all' && (
                    <button onClick={() => setMode('add')} className={combine(getPrimaryButtonClass(), "mt-2")}>
                      Add First Staff
                    </button>
                  )}
                </td>
              ...
              currentStaff.map((member) => {
                const RoleIcon = getRoleIcon(member.role);
                return (
                  <tr key={member.id} className="transition-colors duration-150 hover:bg-[var(--color-bg-hover)]">
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className={combine("font-medium text-xs sm:text-sm", get('accent', 'primary'))}>{member.staff_id}</div>
                    </td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="flex items-center">
                        <div className={combine("h-9 w-9 rounded-full flex items-center justify-center mr-3", theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100')}>
                          <FaUserTie className={combine("text-sm", theme === 'dark' ? 'text-gray-400' : 'text-gray-600')} />
                        </div>
                        <div>
                          <div className="font-semibold text-xs sm:text-sm">{member.name}</div>
                          {member.extra_details?.gender && <div className={combine("text-xs mt-0.5", get('text', 'tertiary'))}>{member.extra_details.gender}</div>}
                        </div>
                      </div>
                    </td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={getStatusBadgeClass(member.role)}>{formatRoleDisplay(member.role)}</span>
                      </div>
                    </td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                    ...
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="flex space-x-1.5">
                        <button onClick={() => router.push(`/admin/staff/overview/${member.staff_id}`)} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-primary)]", get('icon', 'primary') + " text-sm")} title="View Profile"><FaEye className="text-sm" /></button>
                        <button onClick={() => router.push(`/admin/staff/attendance?staffId=${member.staff_id}&tab=history&redirectedFrom=staff-directory`)} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-info)]", get('icon', 'primary') + " text-sm")} title="View Attendance History"><FaHistory className="text-sm" /></button>
                        <button onClick={() => { const staffType = normalizeStaffType(member.role); router.push(`/admin/staff/resources?search=${encodeURIComponent(member.name)}&staffType=${encodeURIComponent(staffType)}&redirectedFrom=staff-directory`); }} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-warning)]", get('icon', 'primary') + " text-sm")} title="View Inventory Usage"><FaBox className="text-sm" /></button>
                        <button onClick={() => router.push(`/admin/staff/work?search=${encodeURIComponent(member.name)}&filterStaffType=${member.role}&redirectedFrom=staff-directory`)} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-warning)]", get('icon', 'primary') + " text-sm")} title="View Work Assignments"><FaTasks className="text-sm" /></button>
                        <button onClick={() => { const params = new URLSearchParams(); params.set('tab', 'employeeReport'); params.set('employee_type', 'staff'); params.set('employee_id', member.staff_id); params.set('redirectedFrom', 'staff-directory'); router.push(`/admin/salary?${params.toString()}`); }} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-success)]", get('icon', 'primary') + " text-sm")} title="View Salary Report"><FaMoneyBillWave className="text-sm" /></button>
                        <button onClick={() => startEdit(member)} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-success)]", get('icon', 'primary') + " text-sm")} title="Edit"><FaEdit className="text-sm" /></button>
                        <button onClick={() => setShowDeleteConfirm(member.id)} className={combine("p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-error)]", get('icon', 'primary') + " text-sm")} title="Delete"><FaTrash className="text-sm" /></button>
                      </div>
                    </td>
                  </tr>
                );
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-4">
                    <div className={combine(
                      ...
                      theme === 'dark' ? 'bg-gray-800 border-indigo-900/30' : 'bg-indigo-100 border-indigo-200'
                    )}>
                      <FaUserTie className={combine(
                        "text-3xl",
                        theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600'
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <p className={combine("text-xs", get('text', 'tertiary'))}>Joining Date</p>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <p className={combine("font-medium text-sm mt-1", get('text', 'primary'))}>
                            {selectedStaff.extra_details?.date_of_birth ? new Date(selectedStaff.extra_details.date_of_birth).toLocaleDateString() : 'N/A'}
                          </p>
                        </div>
                        <div>
                          <p className={combine("text-xs", get('text', 'tertiary'))}>Gender</p>
                          <p className={combine("font-medium text-sm mt-1", get('text', 'primary'))}>
                      ...
                      <div>
                        <p className={combine("text-xs", get('text', 'tertiary'))}>Qualification</p>
                        <p className={combine("font-medium text-sm mt-1", get('text', 'primary'))}>
        ...
        {(mode === 'add' || mode === 'edit') && (
          <div className="animate-fade-in max-w-4xl mx-auto">
            <div className={getCardGradientClass()}>
              ...
              {/* Form Content */}
              <form onSubmit={mode === 'add' ? createStaff : updateStaff} noValidate className="space-y-6">
                <div className="space-y-6">
                  {/* Basic Information - Model Fields */}
                  <div>
                    <h3 className={combine("text-sm font-semibold mb-3", get('text', 'primary'))}>Basic Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateFormField('staff_id', e.target.value)}
                          disabled={mode === 'edit'}
                          className={combine(getInputClass(), "disabled:opacity-50")}
                          placeholder="STF001"
                        />
                        {formErrors.staff_id && <p className="mt-1 text-xs text-red-500">{formErrors.staff_id}</p>}
                      </div>
                      <div>
                        <label className={combine("block text-xs font-medium mb-2", get('text', 'primary'))}>
                          Full Name *
                  ...
                  <div>
                    <h3 className={combine("text-sm font-semibold mb-3", get('text', 'primary'))}>Additional Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className={combine("block text-xs font-medium mb-2", get('text', 'primary'))}>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateFormField('salary_grade', e.target.value)}
                          className={getInputClass()}
                          placeholder="Grade 3"
                        />
                      </div>
                  ...
                  <div>
                    <h3 className={combine("text-sm font-semibold mb-3", get('text', 'primary'))}>Bank Details (Optional)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className={combine("block text-xs font-medium mb-2", get('text', 'primary'))}>
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => updateFormField('upi_id', e.target.value)}
                          className={getInputClass()}
              ...
              <div className="text-center">
                ...
                <p className={combine("text-sm mb-4", get('text', 'secondary'))}>
                  {`Are you sure you want to delete ${
                    staff.find((member) => member.id === showDeleteConfirm)?.name || 'this staff member'
