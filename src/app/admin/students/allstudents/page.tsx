...
const [availableSections, setAvailableSections] = useState<string[]>([]);
  ...
  const fetchStudents = async (page: number) => {
    ...
    try {
      ...
      if (filterAssignmentStatus !== 'all') params.assignment_status = filterAssignmentStatus;
  ...
  }, [currentPage, searchTerm, filterGender, filterClass, filterSection, filterAcademicYear, filterAssignmentStatus]);

  useEffect(() => {
    fetchGenderStats();
  }, [searchTerm, filterGender, filterClass, filterSection, filterAcademicYear, filterAssignmentStatus]);
  ...
  if (filterClass === 'all') {
    ...
    if (selectedStandard?.sections) {
      setAvailableSections(selectedStandard.sections.map(s => s.name).sort());
    } else {
...
const handleViewFeeReport = (student: Student) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.student_name.trim()) nextErrors.student_name = 'Student name is required.';
    if (!formData.student_email.trim()) nextErrors.student_email = 'Email is required.';
    if (formData.student_email.trim() && !isValidEmail(formData.student_email)) {
      nextErrors.student_email = 'Enter a valid email address.';
    }
    if (!formData.father_name.trim()) nextErrors.father_name = "Father's name is required.";
    if (!formData.mother_name.trim()) nextErrors.mother_name = "Mother's name is required.";
    if (!formData.father_phone.trim()) nextErrors.father_phone = "Father's phone is required.";
    if (!formData.mother_phone.trim()) nextErrors.mother_phone = "Mother's phone is required.";
    if (formData.father_phone.trim() && !isValidPhoneNumber(formData.father_phone)) {
      nextErrors.father_phone = "Enter a valid father's phone number (10-15 digits).";
    }
    if (formData.mother_phone.trim() && !isValidPhoneNumber(formData.mother_phone)) {
      nextErrors.mother_phone = "Enter a valid mother's phone number (10-15 digits).";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.date_of_admission) payload.date_of_admission = formData.date_of_admission;
  ...
  const handleBulkImageUpload = async () => {
    ...
    try {
      const response = await adminApi.csv.uploadProfileImagesZip('student', imagesZipFile);
      ...
      if (failedCount === 0) {
        toastSuccess(`Profile image upload completed. Updated ${successCount} students.`);
      } else {
        toastWarning(`Profile images processed: ${successCount} updated, ${failedCount} failed.`);
        if (errors.length > 0) {
  ...
  const downloadSampleCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      sampleRow.map((field) => `"${String(field).replace(/"/g, '""')}"`).join(','),
    ].join('\n');
  ...
  const indexOfFirstItem = totalStudentsCount === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  ...
  const deleteStudentName =
    showDeleteConfirm
      ? students.find((s) => s.student_id === showDeleteConfirm)?.student_name || showDeleteConfirm
      : '';
  ...
  const exportToCSV = async () => {
    const headers = [
      'Student ID', 'Name', 'Email', 'Gender', 'Accommodation', 'DOB',
    ...
    try {
      ...
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      toastSuccess(`CSV exported successfully! (${allStudents.length} students)`);
    } catch (error) {
  ...
  const totalClassPages = Math.max(1, Math.ceil(standards.length / cardsPerPage));
      ...
      <div className="mx-auto w-full max-w-[1600px]">
        ...
        <div className="mb-3 sm:mb-4 md:mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <div className="flex flex-nowrap items-center gap-2 sm:gap-3 w-full lg:w-auto overflow-x-auto lg:overflow-visible">
              {mode === 'list' ? (
                <>
                  <button
                    onClick={exportToCSV}
                    className={combine(getSecondaryButtonClass(), "flex items-center space-x-2 shrink-0")}
                  >
                  ...
                  <button
                    ...
                    className={combine(getPrimaryButtonClass(), "flex items-center space-x-2 shrink-0")}
                  >
            ...
            <div className={getStatsCardClass('blue')}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={combine("text-lg sm:text-xl md:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>{totalStudents}</p>
                </div>
                <div className={combine(
              ...
              <div className={combine("mt-2 sm:mt-3 md:mt-4 text-xs", get('text', 'tertiary'))}>
            ...
            <div className={getStatsCardClass('emerald')}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={combine("text-lg sm:text-xl md:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>{maleCount}</p>
                </div>
                <div className={combine(
                  "p-1.5 sm:p-2 rounded-lg sm:rounded-xl",
            ...
            <div className={getStatsCardClass('pink')}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={combine("text-lg sm:text-xl md:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>{femaleCount}</p>
                </div>
                <div className={combine(
                  "p-1.5 sm:p-2 rounded-lg sm:rounded-xl",
            ...
            <div className={getStatsCardClass('amber')}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={combine("text-lg sm:text-xl md:text-2xl font-bold mt-1 sm:mt-2", get('text', 'primary'))}>{uniqueClasses.length}</p>
                </div>
                <div className={combine(
                  "p-1.5 sm:p-2 rounded-lg sm:rounded-xl",
              ...
              <div className={getCardGradientClass('emerald')}>
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Bulk Upload Students</h3>
                  ...
                  >
                    <FaTimesCircle className={get('icon', 'secondary') + " text-xs sm:text-sm"} />
                  </button>
                </div>
                ...
                <div className="mb-3 sm:mb-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                    ...
                    >
                      ...
                      <span className="text-xs sm:text-sm">Download Sample CSV</span>
                    </button>
                    <button
                ...
                <div className={combine("mb-3 sm:mb-4 pt-3 border-t", get('border', 'primary'))}>
                  <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                    ...
                    >
                      <FiDownload className="text-xs sm:text-sm" />
                      <span className="text-xs sm:text-sm">Upload Images ZIP</span>
                    </button>
                  </div>
                  {imagesZipFile && (
                    <p className={combine(
                      ...
                      <FiCheckCircle className="text-xs sm:text-sm" />
                      Selected ZIP: <span className="truncate">{imagesZipFile.name}</span>
                    </p>
                  )}
                  <p className={combine("mt-1 text-xs", get('text', 'tertiary'))}>
                    ZIP should contain image files named as student IDs (e.g., STU001.jpg, STU002.png).
                  </p>
                </div>

                {uploadProgress > 0 && uploadProgress < 100 && (
                  <div className="mb-3 sm:mb-4">
                    <div className={combine("flex justify-between text-xs mb-0.5 sm:mb-1", get('text', 'secondary'))}>
                      <span>Uploading...</span>
                      <span>{uploadProgress}%</span>
                    </div>
                    <div className={combine("w-full rounded-full h-1 sm:h-1.5", get('bg', 'secondary'))}>
                      <div
                        className={combine(
                ...
                <div className={combine("text-xs", get('text', 'secondary'))}>
                  <p className={combine("font-medium mb-1 sm:mb-2", get('text', 'primary'))}>CSV Format:</p>
                  <div className={combine(
            ...
            <div className={getCardGradientClass('indigo')}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Class Distribution</h3>
                    <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
                ...
                {standards.length > cardsPerPage && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setClassCarouselPage(prev => Math.min(prev + 1, totalClassPages - 1))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3">
                {standards.length > 0 ? visibleStandards.map((standard) => {
                  const classStat = classStats.find((c) => c.class === standard.name);
                  const studentCount = classStat?.count || 0;
                  const sections = standard.sections?.map(s => s.name) || [];
                  const barWidth = totalStudentsCount > 0 ? (studentCount / totalStudentsCount) * 100 : 0;
                  ...
                  return (
                    <div key={standard.id} className={combine(
                      ...
                      <div className="flex items-center justify-between mb-1 sm:mb-2">
                        <span className={combine("font-bold text-xs sm:text-sm truncate", get('text', 'primary'))}>
                          Class {standard.name}
                        </span>
                  ...
                  <div className={combine(
                    ...
                    <p className="text-xs sm:text-sm font-medium">No classes available</p>
                    <p className="text-xs mt-1">Class distribution will appear once classes are configured.</p>
                  </div>
                )}
              </div>
              {standards.length > cardsPerPage && (
  ...
  <div>
    ...
    >
      <option value="">{academicYearsLoading ? 'Loading academic years...' : 'Select Academic Year'}</option>
  ...
  <div>
    ...
    <select
      ...
      onChange={(e) => {
        setFilterAssignmentStatus(e.target.value as 'all' | 'assigned' | 'unassigned');
        setCurrentPage(1);
      }}
      className={getInputClass()}
    >
      ...
      <option value="unassigned">Not Class Assigned Student</option>
    </select>
  </div>
  ...
  <div>
    ...
    >
      ...
      {standards.length > 0 && standards.map(std => (
        <option key={std.id} value={std.name}>Class {std.name}</option>
      ))}
    </select>
  </div>
  ...
  <div>
    ...
    >
      <option value="all">
        {filterClass === 'all' ? 'All Sections' : `All Sections in Class ${filterClass}`}
      </option>
...
<div className={getCardGradientClass()}>
  <div className={combine(
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
      <div>
        <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Student Records</h3>
        <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
      ...
      <div className={combine("text-xs", get('text', 'tertiary'))}>
        Showing {indexOfFirstItem} to {indexOfLastItem} of {totalStudents} students
      </div>
    </div>
  </div>
      ...
      {loading ? (
        <div className="p-6 sm:p-8 min-h-[280px] flex items-center justify-center text-center">
          <div className="text-center">
            ...
            <p className={combine("mt-4 sm:mt-6 text-xs sm:text-sm font-medium", get('text', 'secondary'))}>Loading students...</p>
            <p className={combine("text-xs sm:text-sm mt-1 sm:mt-2", get('text', 'tertiary'))}>Preparing student records</p>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-8 text-center">
          ...
          <h3 className={combine("text-sm sm:text-base font-medium mb-0.5 sm:mb-1", get('text', 'primary'))}>No students found</h3>
          <p className={combine("text-xs sm:text-sm mb-3 sm:mb-4", get('text', 'secondary'))}>
            {searchTerm || filterGender !== 'all' || filterClass !== 'all' || !!filterAcademicYear
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className={combine("font-semibold text-sm", get('text', 'primary'))}>{student.student_name}</p>
                  <p className={combine("text-xs mt-0.5", get('text', 'tertiary'))}>ID: {student.student_id}</p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    onClick={() => router.push(`/admin/students/overview/${student.student_id}`)}
                    className={combine("p-1.5 rounded-lg", get('icon', 'primary'))}
                    title="View Details"
                  >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                <div>
                  <p className={combine("font-medium", get('text', 'tertiary'))}>Class</p>
                  <p className={combine("mt-0.5", get('text', 'secondary'))}>
                    {student.class_name || 'Not Assigned'}{student.section ? ` - ${student.section}` : ''}
                  </p>
                </div>
                <div>
                  <p className={combine("font-medium", get('text', 'tertiary'))}>Admission</p>
                  <p className={combine("mt-0.5", get('text', 'secondary'))}>{student.date_of_admission || 'Not Provided'}</p>
                </div>
                <div className="col-span-2">
                  <p className={combine("font-medium", get('text', 'tertiary'))}>Contact</p>
                  <p className={combine("mt-0.5", get('text', 'secondary'))}>
                    {student.father_phone || student.mother_phone || 'Not Provided'}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className={combine("font-medium", get('text', 'tertiary'))}>Address</p>
                  <p className={combine("mt-0.5 truncate", get('text', 'secondary'))}>
      ...
      <div className="overflow-y-auto">
        <table className="w-full border-collapse text-xs sm:text-sm">
              ...
              {shouldShowColumn('class_info') && (
                ...
                >
                  <div className="flex items-center space-x-1 sm:space-x-2">
                    ...
                    <span className="text-xs sm:text-sm">Class</span>
                    <div className="ml-1">
              ...
              {shouldShowColumn('date_of_admission') && (
                ...
                >
                  <div className="flex items-center space-x-1 sm:space-x-2">
                    ...
                    <span className="text-xs sm:text-sm">Admission Date</span>
                    <div className="ml-1">
          ...
          <tbody className={combine("divide-y", get('border', 'primary'))}>
            {loading ? (
              <tr>
                <td colSpan={7} className="px-3 sm:px-4 py-8 h-[280px] align-middle text-center">
                  <div className="h-full flex flex-col items-center justify-center text-center">
                    ...
                    <p className={combine("mt-6 text-sm font-medium", get('text', 'secondary'))}>Loading students...</p>
                    <p className={combine("text-sm mt-2", get('text', 'tertiary'))}>Preparing student records</p>
                  </div>
                </td>
              </tr>
            ) : currentStudents.length === 0 ? (
                ...
                >
                    ...
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="flex items-center">
                        <div className={combine(
                          ...
                          <FaIdCard className={
                            student.gender === 'Male'
                              ? theme === 'dark' ? 'text-blue-400 text-xs sm:text-sm' : 'text-blue-600 text-xs sm:text-sm'
                              : theme === 'dark' ? 'text-pink-400 text-xs sm:text-sm' : 'text-pink-600 text-xs sm:text-sm'
                          } />
                        </div>
                        <div>
                          <h4 className={combine("font-semibold text-xs sm:text-sm md:text-sm", get('text', 'primary'))}>
                            {student.student_id}
                          </h4>
                        </div>
                      </div>
                  ...
                  {shouldShowColumn('student_name') && (
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="space-y-0.5 sm:space-y-1">
                        <div className='flex items-center'>
                          <h4 className={combine("font-semibold text-xs sm:text-sm md:text-sm mx-1 sm:mx-2", get('text', 'primary'))}>
                            {student.student_name}
                          </h4>
                        </div>
                      </div>
                  ...
                  {shouldShowColumn('class_info') && (
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="space-y-0.5 sm:space-y-1">
                        <div className="flex items-center space-x-1 sm:space-x-2">
                          <span className={combine("font-medium text-xs sm:text-sm md:text-sm", get('text', 'primary'))}>
                  ...
                  {shouldShowColumn('address') && (
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="space-y-0.5 sm:space-y-1">
                        {student.address ? (
                          <div>
                            <p className={combine("text-xs sm:text-sm truncate max-w-[120px] sm:max-w-[180px]", get('text', 'secondary'))}>
                              {student.address}
                            </p>
                          </div>
                        ) : (
                          <span className={combine("text-xs sm:text-sm italic", get('text', 'tertiary'))}>
                            Not provided
                          </span>
                        )}
                      </div>
                  ...
                  {shouldShowColumn('contact_info') && (
                    <td className="px-3 sm:px-4 py-2 sm:py-3">
                      <div className="space-y-0.5 sm:space-y-1">
                        <div>
                          {student.father_phone && (
                            <p className={combine("text-xs sm:text-sm flex items-center space-x-1", get('text', 'secondary'))}>
                              <FaPhone className="text-xs" />
                              <span className="text-xs sm:text-sm truncate max-w-[80px] sm:max-w-[120px]">{student.father_phone}</span>
                            </p>
                          )}
                          {student.mother_phone && (
                            <p className={combine("text-xs sm:text-sm flex items-center space-x-1 mt-0.5 sm:mt-1", get('text', 'secondary'))}>
                              <FaPhone className="text-xs" />
                              <span className="text-xs sm:text-sm truncate max-w-[80px] sm:max-w-[120px]">{student.mother_phone}</span>
                            </p>
                          )}
                        </div>
                      </div>
  ...
  <td className="px-3 sm:px-4 py-2 sm:py-3">
    <div className="flex space-x-1 sm:space-x-1.5">
      ...
      <button
        onClick={() => handleViewFeeReport(student)}
        className={combine(
          "p-1 sm:p-1.5 rounded-lg sm:rounded-xl transition-all duration-200 hover:text-[var(--color-accent-primary)]",
          get('icon', 'primary') + " text-xs sm:text-sm"
        )}
            ...
            <div className={getCardGradientClass()}>
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-3 sm:space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {/* Student ID */}
                  <div>
                    <label className={combine("block text-xs sm:text-sm font-medium mb-1 sm:mb-1.5", get('text', 'primary'))}>
                    ...
                    <input
                      ...
                      placeholder="STU001"
                    />
                    {formErrors.student_id && <p className="mt-1 text-xs text-red-500">{formErrors.student_id}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {formErrors.student_name && <p className="mt-1 text-xs text-red-500">{formErrors.student_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {formErrors.student_email && <p className="mt-1 text-xs text-red-500">{formErrors.student_email}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {formErrors.father_name && <p className="mt-1 text-xs text-red-500">{formErrors.father_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {formErrors.mother_name && <p className="mt-1 text-xs text-red-500">{formErrors.mother_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {formErrors.father_phone && <p className="mt-1 text-xs text-red-500">{formErrors.father_phone}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="+91 9876543210"
                    />
                    {formErrors.mother_phone && <p className="mt-1 text-xs text-red-500">{formErrors.mother_phone}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {formErrors.class_name && <p className="mt-1 text-xs text-red-500">{formErrors.class_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {formData.class_name && standards.length > 0 &&
                        (() => {
                          const selectedStandard = standards.find(std => std.name === formData.class_name);
                          return selectedStandard?.sections?.map(section => (
                    ...
                    {formErrors.section && <p className="mt-1 text-xs text-red-500">{formErrors.section}</p>}
                  </div>
                </div>
                ...
                {/* Form Actions */}
                <div className={combine("flex flex-col sm:flex-row gap-2 sm:space-x-2 pt-3 sm:pt-4 border-t", get('border', 'primary'))}>
                  <button
                    type="button"
                    onClick={() => {
                      setMode('list');
                  ...
                  >
                    {loading ? (
                      <>
                        ...
                        <span className="text-xs sm:text-sm">{mode === 'edit' ? 'Updating...' : 'Saving...'}</span>
                      </>
                    ) : (
                      <>
                        <FaSave className="text-xs sm:text-sm" />
                        <span className="text-xs sm:text-sm">{mode === 'edit' ? 'Update Student' : 'Save Student'}</span>
                      </>
                    )}
                  </button>
                </div>
            ...
            <div className={combine(
              ...
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  ...
                  <div>
                    <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Student Details</h3>
                    <p className={combine("text-xs sm:text-sm mt-0.5", get('text', 'secondary'))}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                ...
                <div className={combine(
                  ...
                  <h4 className={combine("font-semibold text-xs sm:text-sm mb-2 sm:mb-3 flex items-center space-x-1.5 sm:space-x-2", get('text', 'primary'))}>
                    <FaUser className={theme === 'dark' ? 'text-blue-400 text-xs sm:text-sm' : 'text-blue-500 text-xs sm:text-sm'} />
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.student_id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className={combine("text-xs", get('text', 'tertiary'))}>Full Name</span>
                      <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.student_name}</span>
                    </div>
                    ...
                    {selectedStudent.accommodation && (
                      <div className="flex justify-between">
                        <span className={combine("text-xs", get('text', 'tertiary'))}>Accommodation</span>
                        <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>
                          {selectedStudent.accommodation === 'day_scholar' ? 'Day Scholar' : 'Hosteller'}
                        </span>
                      </div>
                    )}
                    {selectedStudent.date_of_birth && (
                      <div className="flex justify-between">
                        ...
                        <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.date_of_birth}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className={combine(
                  ...
                  <h4 className={combine("font-semibold text-xs sm:text-sm mb-2 sm:mb-3 flex items-center space-x-1.5 sm:space-x-2", get('text', 'primary'))}>
                    <FaSchool className={theme === 'dark' ? 'text-emerald-400 text-xs sm:text-sm' : 'text-emerald-500 text-xs sm:text-sm'} />
                    <span className="text-xs sm:text-sm">Academic Information</span>
                  </h4>
                  <div className="space-y-1.5 sm:space-y-2">
                    <div className="flex justify-between">
                    ...
                    {selectedStudent.student_email && (
                      <div className="flex justify-between">
                        ...
                        <span className={combine("font-medium text-xs sm:text-sm", get('accent', 'primary'))}>
                          {selectedStudent.student_email}
                        </span>
                      </div>
                    )}
                ...
                <div className="md:col-span-2">
                  <div className={combine(
                    ...
                    <h4 className={combine("font-semibold text-xs sm:text-sm mb-2 sm:mb-3 flex items-center space-x-1.5 sm:space-x-2", get('text', 'primary'))}>
                      <MdOutlineFamilyRestroom className={theme === 'dark' ? 'text-purple-400 text-xs sm:text-sm' : 'text-purple-500 text-xs sm:text-sm'} />
                      <span className="text-xs sm:text-sm">Parent Information</span>
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <div className="space-y-1.5 sm:space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.father_name}</span>
                          </div>
                          {selectedStudent.father_phone && (
                            <div className="flex justify-between">
                              <span className={combine("text-xs", get('text', 'tertiary'))}>Father's Phone</span>
                              <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.father_phone}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <div>
                        <div className="space-y-1.5 sm:space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.mother_name}</span>
                          </div>
                          {selectedStudent.mother_phone && (
                            <div className="flex justify-between">
                              <span className={combine("text-xs", get('text', 'tertiary'))}>Mother's Phone</span>
                              <span className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.mother_phone}</span>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {selectedStudent.address && (
                <div className={combine(
                  ...
                  <h4 className={combine("font-semibold text-xs sm:text-sm mb-2 sm:mb-3 flex items-center space-x-1.5 sm:space-x-2", get('text', 'primary'))}>
                    <FaMapMarkerAlt className={theme === 'dark' ? 'text-amber-400 text-xs sm:text-sm' : 'text-amber-500 text-xs sm:text-sm'} />
                    <span className="text-xs sm:text-sm">Address</span>
                  </h4>
                  <p className={combine("font-medium text-xs sm:text-sm", get('text', 'primary'))}>{selectedStudent.address}</p>
                </div>
              )}
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-2 sm:p-4 z-50 animate-fade-in backdrop-blur-sm">
            <div className={combine(
              ...
              <div className="text-center p-4 sm:p-6">
                ...
                <h3 className={combine("text-base sm:text-lg font-bold mb-1 sm:mb-1.5", get('text', 'primary'))}>Delete Student</h3>
                <p className={combine("text-xs sm:text-sm mb-3 sm:mb-4", get('text', 'secondary'))}>
                  Are you sure you want to delete <span className={combine("font-semibold", get('text', 'primary'))}>{deleteStudentName}</span>? This action cannot be undone.
                </p>
