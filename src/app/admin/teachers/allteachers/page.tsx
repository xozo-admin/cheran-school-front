...
export default function AllTeachersPage() {
  ...
  const fetchTeachers = async (page: number) => {
    ...
    try {
      ...
      if (filterSection && filterSection !== 'all') params.section = filterSection;
  ...
  const fetchTeacherStats = async () => {
    try {
      ...
      const deptCount = Array.from(new Set(allTeachers.map((t) => t.department).filter(Boolean))).length;
  ...
  }, [currentPage, searchTerm, filterDept, filterStatus, filterClass, filterSection]);

  useEffect(() => {
    fetchTeacherStats();
  }, [searchTerm, filterDept, filterStatus, filterClass, filterSection]);
  ...
  const validateForm = () => {
    ...
    if (!formData.teacher_id.trim()) nextErrors.teacher_id = 'Teacher ID is required.';
    ...
    if ((formData.class_name.trim() && !formData.section.trim()) || (!formData.class_name.trim() && formData.section.trim())) {
      if (!formData.class_name.trim()) nextErrors.class_name = 'Class is required when section is provided.';
      if (!formData.section.trim()) nextErrors.section = 'Section is required when class is provided.';
    }
  ...
  const indexOfFirstItem = totalTeachersCount === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  ...
  const exportToCSV = async () => {
    const headers = ['Teacher ID', 'Name', 'Email', 'Phone', 'Qualification', 'Department', 'DOB', 'Joining Date', 'Assigned Class', 'Address'];
    ...
    try {
      ...
      toastSuccess(`CSV exported successfully! (${allTeachers.length} teachers)`);
    } catch (error) {
  ...
  const handleBulkImageUpload = async () => {
    ...
    try {
      const response = await adminApi.csv.uploadProfileImagesZip('teacher', imagesZipFile);
      ...
      if (failedCount === 0) {
        toastSuccess(`Profile image upload completed. Updated ${successCount} teachers.`);
      } else {
                ...
                <div className={getCardGradientClass('emerald')}>
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Bulk Upload Teachers</h3>
                        <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
                  ...
                  <div className={combine("mb-3 sm:mb-4 pt-3 border-t", get('border', 'primary'))}>
                    ...
                    <p className={combine("mt-1 text-xs", get('text', 'tertiary'))}>
                      ZIP should contain image files named as teacher IDs (e.g., TCH001.jpg, TCH002.png).
                    </p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3 sm:gap-4">
                  ...
                  >
                    {departments.map(dept => (
                      <option key={dept} value={dept}>
                        {dept === 'all' ? 'All Departments' : dept.length > 15 ? dept.substring(0, 12) + '...' : dept}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {sectionOptions.filter(s => s !== 'all').map(sectionName => (
            ...
            <div className={getCardGradientClass()}>
              ...
              <div className={combine("p-4 border-b", get('border', 'primary'))}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
                  <div>
                    <h3 className={combine("text-base sm:text-lg font-bold", get('text', 'primary'))}>Teacher Records</h3>
                    <p className={combine("text-xs sm:text-sm mt-0.5 sm:mt-1", get('text', 'secondary'))}>
                  ...
                  <div className={combine("text-xs", get('text', 'tertiary'))}>
                    Showing {indexOfFirstItem} to {indexOfLastItem} of {totalTeachers}
                  </div>
                </div>
              </div>
                ...
                {loading ? (
                  <div className="p-6 sm:p-8 min-h-[280px] flex items-center justify-center text-center">
                    <div className="text-center">
                      ...
                      <p className={combine("mt-4 sm:mt-6 text-xs sm:text-sm font-medium", get('text', 'secondary'))}>Loading teachers...</p>
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <p className={combine("font-semibold text-sm sm:text-base truncate", get('text', 'primary'))}>{teacher.name}</p>
                                <p className={combine("text-xs mt-0.5", get('text', 'tertiary'))}>ID: {teacher.teacher_id}</p>
                                <p className={combine("text-xs mt-0.5 truncate", get('text', 'secondary'))}>{teacher.qualification}</p>
                              </div>
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-2 text-xs sm:text-sm">
                              <div className="col-span-2">
                                ...
                                <p className={combine("mt-0.5 truncate", get('text', 'secondary'))}>{teacher.email}</p>
                                <p className={combine("mt-0.5", get('text', 'secondary'))}>{teacher.phone}</p>
                              </div>
                              <div>
                                <p className={combine("font-medium", get('text', 'tertiary'))}>Joining</p>
                                <p className={combine("mt-0.5", get('text', 'secondary'))}>{teacher.joining_date || 'Not specified'}</p>
                              </div>
                              <div>
                                <p className={combine("font-medium", get('text', 'tertiary'))}>Class Status</p>
                                <p className={combine("mt-0.5", get('text', 'secondary'))}>
                                  {teacher.assigned_class && teacher.assigned_class !== 'Not Assigned' ? `Class Teacher: ${teacher.assigned_class}` : 'Not Assigned'}
                                </p>
                              </div>
                            </div>
                              ...
                              <div className="mt-2">
                                <button
                                  onClick={() => {
                                    ...
                                    router.push(`/admin/teachers/allocations?${params.toString()}`);
                                  }}
                                  className={combine(
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-1.5">
                              <button
                                onClick={() => router.push(`/admin/teachers/overview/${teacher.teacher_id}`)}
                                className={combine(
                              ...
                              <button
                                onClick={() => router.push(`/admin/teachers/attendance?teacherId=${teacher.teacher_id}&tab=history&redirectedFrom=allteachers`)}
                                className={combine(
                                  "p-1.5 rounded-lg transition-all duration-200 hover:text-[var(--color-accent-info)]",
                                  get('icon', 'primary') + " text-xs"
                                )}
                        ...
                        <tr>
                          ...
                          >
                            <div className="flex items-center space-x-1 sm:space-x-2">
                              ...
                              <span className="text-xs sm:text-sm">Teacher ID</span>
                              <div className="ml-1">
                          ...
                          >
                            <div className="flex items-center space-x-1 sm:space-x-2">
                              ...
                              <span className="text-xs sm:text-sm">Teacher Details</span>
                              <div className="ml-1">
                                {sortField === 'name' ? (
                                  sortDirection === 'asc' ?
                                    <FaSortUp className={get('accent', 'primary') + " text-xs"} /> :
                          ...
                          >
                            ...
                            <td className="px-3 sm:px-4 py-2 sm:py-3">
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <div className={combine("text-xs mt-0.5 flex items-center space-x-1", get('text', 'tertiary'))}>
                            ...
                            <td className="px-3 sm:px-4 py-2 sm:py-3">
                              <div className="space-y-1">
                                <div className={combine("flex items-center text-sm", get('text', 'primary'))}>
                                  <FaEnvelope className={combine("mr-1 sm:mr-2 text-xs", get('icon', 'secondary'))} />
                                  <span className="truncate max-w-[100px] sm:max-w-[120px] text-sm">{teacher.email}</span>
                                </div>
                                <div className={combine("flex items-center text-sm", get('text', 'primary'))}>
                                  <FaPhone className={combine("mr-1 sm:mr-2 text-xs", get('icon', 'secondary'))} />
        ...
        {(mode === 'add' || mode === 'edit') && (
          <div className="animate-fade-in max-w-4xl mx-auto">
            <div className={getCardGradientClass()}>
              ...
              {/* Form Content */}
              <form onSubmit={handleSubmit} noValidate className="space-y-4 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  {/* Basic Information */}
                  <div className="col-span-2">
                    <h3 className={combine("text-base sm:text-lg font-semibold mb-3 sm:mb-4", get('text', 'primary'))}>Basic Information</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="TCH001"
                        />
                        {formErrors.teacher_id && <p className="mt-1 text-xs text-red-500">{formErrors.teacher_id}</p>}
                      </div>
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                  ...
                  {/* Contact Information */}
                  <div className="col-span-2">
                    <h3 className={combine("text-base sm:text-lg font-semibold mb-3 sm:mb-4", get('text', 'primary'))}>Contact Information</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                  ...
                  <div className="col-span-2">
                    <h3 className={combine("text-base sm:text-lg font-semibold mb-3 sm:mb-4", get('text', 'primary'))}>Professional Information</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                        ...
                        />
                        {formErrors.qualification && <p className="mt-1 text-xs text-red-500">{formErrors.qualification}</p>}
                      </div>
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                  ...
                  {/* Additional Details */}
                  <div className="col-span-2">
                    <h3 className={combine("text-base sm:text-lg font-semibold mb-3 sm:mb-4", get('text', 'primary'))}>Additional Details</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                  ...
                  <div className="col-span-2">
                    <h3 className={combine("text-base sm:text-lg font-semibold mb-3 sm:mb-4", get('text', 'primary'))}>
                      Class Teacher Assignment {mode === 'edit' && '(Optional - Leave empty to keep current)'}
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <label className={combine("block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2", get('text', 'primary'))}>
                ...
                {/* Form Actions */}
                <div className={combine("flex space-x-2 space-x-3 pt-4 sm:pt-6 border-t", get('border', 'primary'))}>
                  <button
                    type="button"
                    onClick={() => {
                      setFormErrors({});
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span className="text-xs sm:text-sm">{mode === 'edit' ? 'Update Teacher' : 'Save Teacher'}</span>
                      </>
                    )}
                  </button>
                </div>
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-3 sm:p-4 z-50 animate-fade-in backdrop-blur-sm">
            <div className={combine(
              ...
              <div className="text-center">
                ...
                <h3 className={combine("text-base sm:text-lg font-bold mb-1 sm:mb-1.5", get('text', 'primary'))}>Delete Teacher</h3>
                <p className={combine("text-xs sm:text-sm mb-3 sm:mb-4", get('text', 'secondary'))}>
                  {`Are you sure you want to delete ${
                    teachers.find((t) => t.id === showDeleteConfirm)?.name || 'this teacher'
